...
static int xilinx_dma_alloc_chan_resources(struct dma_chan *dchan)
{
    ...
    printk("channel %p------------------xilinx_dma_alloc_chan_resources------------\n",chan);
	...
	if (chan->xdev->dma_config->dmatype == XDMA_TYPE_AXIDMA) {
		...
		chan->seg_mv = dma_alloc_coherent(chan->dev,
						  ...
						  &chan->seg_p, GFP_KERNEL);
        printk("channel %p chan->seg_p = %#llx, sizeof(*chan->seg_mv) =  %#llx\n", chan, chan->seg_p , sizeof(*chan->seg_mv));
		if (!chan->seg_mv) {
			dev_err(chan->dev,
				"unable to allocate channel %d descriptors\n",
				chan->id);
		...
		for (i = 0; i < XILINX_DMA_NUM_DESCS; i++) {
            ...
            printk("channel %p chan->seg_mv[%d].hw.next_desc = %#llx, chan->seg_mv[%d].phys =  %#llx\n", chan, 
                    i, chan->seg_mv[i].hw.next_desc, i,chan->seg_mv[i].phys);
		}
	} else if (chan->xdev->dma_config->dmatype == XDMA_TYPE_CDMA) {
